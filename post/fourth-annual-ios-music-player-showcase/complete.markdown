...
* **Importing from Cloud services** via the Files app picker modal, so long as the cloud service you want to import from is present in the Files app as a "Location" (all major cloud services like [Dropbox](https://www.dropbox.com), [Onedrive](http://onedrive.live.com), etc. support this). This isn't the best overall method since you can only import manually selected individual audio files this way, but it's a nice option to have if you just need to quickly import a couple songs or a new album.
...
Picky this year also released rich multi-disc album support. Not only does each disk in a multi-disc album receive proper labeling and separation, you can also discretely play, shuffle, or add to queue the discs individually. For example, if you tap the play button on "Disc 1" of a particular multi-disc album, playback would stop once all contents are played on that disc and would *not* automatically proceed to the rest of the album's discs like with standard playback. While this is an exceptionally niche feature, Picky appears to be the only general-purpose player offering individual disc control like this, and it's absolutely wonderful for listeners with sprawling albums containing unique and independent discs. A somewhat recent example of this is [Kamasi Washinton](https://music.apple.com/us/artist/kamasi-washington/154076564)'s [*The Epic*](https://music.apple.com/us/album/the-epic/975610456), a 3-hour long jazz behemoth with three discrete album-length discs. In this particular case, separation of these discrete parts is sometimes ideal, if for no other reason than to provide a purposeful pause as an audible "palette clenser" before beginning the next act. While some niche players like [Vinyl Fetish] also offer this separation, Picky is the first and only general-purpose player to do so, which by default makes it the ideal general-purpose player for playback of extra-long records or records where per-disc separation is desired.
